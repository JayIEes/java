...
public class UseFileWriter1 {
	...
	public void writeData() throws IOException {
		String msg="오늘은 즐거운 금요일 입니다.3";
		
		File file=new File("e:/dev/temp/write_data1.txt");
		//파일이 존재하면 ConfirmDialog로 덮어쓸 것이지 물어보고 "예" 가 눌리는 경우에만 
		//덮어 쓴다. 파일이 존재하지 않으면 파일을 생성한다. => 아래의 코드를 제어하기 위한 flag 변수를 만들어 사용
		boolean createFlag=false;
		
		if(file.exists()) {
			switch (JOptionPane.showConfirmDialog(null, file.getAbsolutePath()
					+"이 존재합니다. 덮어쓰시겠습니까?")) {
